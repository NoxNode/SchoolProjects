...
int TCPSocket::readFromSocketWithTimeout (char * buffer, int maxBytes, long timeoutSec, long timeoutMicro)
{
    ...
    if ( select_result >= 0 ) // if select results is bigger than 0 this means that data is available
    {
        ...
        {
            // we made this change - replaced recv with readFromSocket
                // this makes sure the buffer is zerod out before reading and reuses code
            read = readFromSocket(buffer,maxBytes);
