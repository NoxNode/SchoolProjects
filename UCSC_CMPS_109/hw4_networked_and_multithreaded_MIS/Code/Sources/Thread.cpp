...
void Thread::cleanup(void * target_thread) 
{
    ...
    me->termination_request = true; // Set termination request to true
    int retVal = pthread_mutex_unlock(&me->mutex);   // Unlock the execution management mutex
    if(retVal != 0)
        throw Error("error", 7, "pthread_mutex_unlock failed");
}
...
Thread::Thread(void *(*_threadRoutine) (void *))
{
    ...
    int retVal = pthread_mutex_init (&mutex,NULL);// Initialize execution control mutex
    if(retVal != 0)
        throw Error("error", 7, "failed to init mutex");
    ...
    retVal = pthread_attr_init(&pthread_attr);   // Initialize pthread attributes
    if(retVal != 0)
        throw Error("error", 7, "pthread_attr_init failed");
    // Set thread as dettached at creation time
    retVal = pthread_attr_setdetachstate (&pthread_attr,PTHREAD_CREATE_DETACHED);
    if(retVal != 0)
        throw Error("error", 7, "pthread_attr_setdetachstate failed");
    retVal = pthread_attr_setstacksize(&pthread_attr, stacksize ); // Set new stack size
    if(retVal != 0)
        throw Error("error", 7, "pthread_attr_setstacksize failed");
...
void Thread::start()
{
    int retVal = pthread_mutex_lock(&mutex); // Acquire lock before forking the thread
    if(retVal != 0)
        throw Error("error", 7, "pthread_mutex_lock failed");
...
{
	int retVal = pthread_mutex_lock(&mutex); // Lock mutex which will block in case the thread is running
    if(retVal != 0)
        throw Error("error", 7, "pthread_mutex_lock failed");
	retVal = pthread_mutex_unlock(&mutex);   // Unlock mutex as soon as it is acquired,which guarantees that the thread terminated
    if(retVal != 0)
        throw Error("error", 7, "pthread_mutex_unlock failed");
}
